...
cell AMX_NATIVE_CALL rf_config(AMX *amx, cell *params) {
    ...
    std::wstring name = ws_conv(getAmxString(amx, params[arg_name], buff)).get();

    if (name.empty())

        name = ws_conv(plugin->getName()).get();

    name.replace(name.find(L".amxx"), sizeof(L".amxx") - 1, L"");

    std::wstring path = ws_conv(getAmxString(amx, params[arg_folder], buff)).get();
