...
cell AMX_NATIVE_CALL rf_sql_tuple(AMX *amx, cell *params)
{
    ...
    return 1;//g_mysql_mngr.add_connect(-1, db_host.c_str(), db_user.c_str(), db_pass.c_str(), db_name.c_str(), params[arg_timeout], params[arg_nonblock]);
}
...
cell AMX_NATIVE_CALL rf_sql_async_query(AMX *amx, cell *params)
{
    ...
    return 1;//g_mysql_mngr.push_query(*getAmxAddr(amx, params[arg_tuple]), query, getAmxAddr(amx, params[arg_tuple]), params[arg_data_size], params[arg_pri]);
}
