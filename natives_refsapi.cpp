...
cell AMX_NATIVE_CALL rf_config(AMX *amx, cell *params) {
    ...
    std::wstring name = convert(getAmxString(amx, params[arg_name], buff));

    if (name.empty())

        name = convert(plugin->getName());

    name.replace(name.find(L".amxx"), sizeof(L".amxx") - 1, L"");

    std::wstring path = convert(getAmxString(amx, params[arg_folder], buff));
