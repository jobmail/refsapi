...
cell AMX_NATIVE_CALL rf_config(AMX *amx, cell *params) {
    ...
    std::wstring name = str_to_wstring(getAmxString(amx, params[arg_name], buff));

    if (name.empty())

        name = str_to_wstring(plugin->getName());

    name.replace(name.find(L".amxx"), sizeof(L".amxx") - 1, L"");

    std::wstring path = str_to_wstring(getAmxString(amx, params[arg_folder], buff));
        ...
        if (file.is_open()) {
            ...
            if (is_exist) {
                ...
                while (std::getline(file, line, L'\n')) {
                    ...
                    g_cvar_mngr.add(plugin, var_name, var_value, FCVAR_SPONLY, L"TEST");
                }
