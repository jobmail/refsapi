...
cell AMX_NATIVE_CALL rf_config(AMX *amx, cell *params)
{
    ...
    if ((is_exist = file_exists(path)) || params[arg_auto_create])
    {
        ...
        if (file.is_open())
        {
            ...
            {
                if (g_cvar_mngr.need_update(load_cvars, plugin_cvars))
                {
                    if (file_exists)
                    {
                        file.close();
                    ...
                    UTIL_ServerPrint("[DEBUG] rf_config(): need update config\n");
                    bool need_replace;
                    load_cvars_t::iterator load_cvars_it;
                    m_cvar_t* m_cvar;
                    // Set pos = 0
                    std::filesystem::resize_file(wstos(path).c_str(), 0);
                    file.seekp(0);
                    // Write config
